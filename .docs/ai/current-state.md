...
- **On-device testing + fixes (builds 6/7, 2026-05-29):** build 6 shipped the voice picker + review fixes (uploaded). Device feedback fixed: History sheet stopped opening (`efd121f` — three stacked `.sheet(isPresented:)` shadowed each other → one `.sheet(item:)` enum) and barge-in from Sending/Thinking silently dropped (`8a687fd` — `state=.idle` before `startRecording()`; confirmed by device + a 12-agent verified trace, which also refuted the feared clobber race). **OPEN BLOCKER:** a real turn hung at "transcribing + dispatching to hermes" with no response/timeout — recording+upload succeeded, so it's **server-side** (STT or `hermes.ask`); needs backend-log diagnosis. **Build 7 (the two fixes) NOT cut yet** — the release classifier rightly blocked an unrequested TestFlight upload; awaiting user go-ahead. Feature request logged: live tool-calls during thinking (needs backend streaming).
