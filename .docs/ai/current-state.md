...
- **Builds:** build 7 (`022728f`) = build 6 content + Phase 1 + barge-in fix (`8a687fd`) + History fix (`efd121f`). Build 6 (`a89a966`) already shipped the **voice picker** + the review fixes (quick wins, audio coordinator, voice_id validation). (Build 5 `5b7e786` earlier = redesign/schedules/LiveActivity/Bonjour.) **Phase 2 (live tool-calls) is the only thing committed but NOT in any build yet** — awaits the next release (user authorizes each).
