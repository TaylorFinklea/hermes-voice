...
- [ ] **Conversational voice (the "Jarvis" endgame)** — decomposed after on-device STT landed instant. **Sub-project 1: on-device TTS (Kokoro via FluidAudio) — BUILT (iOS BUILD SUCCEEDED), not cut/device-tested.** On-phone Kokoro (Neural Engine, in the FluidAudio pkg already), sentence-chunked instant playback; voice picker `local:` sentinel; backend `tts=none` = the "text brain" hook for fronting **Claude Code / other harnesses** later. Spec: `.docs/ai/phases/on-device-tts-spec.md`. **Sub-project 2 (next brainstorm): hands-free conversation mode** — VAD turn-taking (listen→endpoint→transcribe→reply→re-listen), no mic press between turns; FluidAudio already ships `VadManager`/`StreamingEouAsrManager`; interrupt via button/mic-tap (vocal barge-in/echo-cancellation deferred).
